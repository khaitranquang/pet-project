...
public class GamePanel extends JPanel {
	...
	public static final Color backGround=new Color(0,158,158);
	public static final int PIPE_H=30, PIPE_W=50;//Cot chuong ngai vat
	...
	public GamePanel(FlappyBird fb, Bird bird, ArrayList<Rectangle> rects){
		...
		this.bird=bird;
		this.rects=rects;
		scoreFont=new Font("Comic Sans MS", Font.BOLD,18);   //Thiet laap phong chu cua diem so
		pauseFont=new Font("Arial", Font.BOLD,48);
		try {
			pipeHead=ImageIO.read(new File("PipeHead.png"));
			pipeLength=ImageIO.read(new File("PipeLength.png"));
		} catch (IOException e) {
	...
	public void paintComponent(Graphics g){
		...
		g.fillRect(0, 0,FlappyBird.WIDTH, FlappyBird.HEIGHT); //To mau hinh chu nhat
		bird.update(g);
		g.setColor(Color.RED);
		for(Rectangle r:rects){
			Graphics2D g2d =(Graphics2D) g; //Ep kieu Graphics2D cho g;
			g2d.setColor(Color.GREEN);
			AffineTransform old=g2d.getTransform();  //Bien doi hinh
			g2d.translate(r.x+PIPE_W/2, r.y+PIPE_H/2);      //tinh tien
